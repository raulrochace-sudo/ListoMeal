...
"""
    ...
    try:
        ...
        print(f"[ListoMeal scan error] {type(e).__name__}: {e}", flush=True)
...
"""
    ...
    try:
        ...
        if image_url:
            return jsonify({"image_data": image_url})

        return jsonify({"error": "Image generation returned no usable image data."}), 500
    except Exception as e:
        print(f"[ListoMeal image error] {type(e).__name__}: {e}", flush=True)
...
"""
    ...
    try:
        ...
        print(f"[ListoMeal recipe error] {type(e).__name__}: {e}", flush=True)
