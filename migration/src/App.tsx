...
const copy = {
  en: {
    ...
    generate: 'Give me 3 meals', scanning: 'Scanning your food…', generating: 'Cooking up ideas…', coming: 'Recipe photo coming soon', cook: 'Cook With Me',
    ...
    noPhoto: 'Photos are being added. The recipe itself is ready to cook.', newRecipe: 'Start New Recipe',
  ...
  es: {
    ...
    coming: 'Foto de receta próximamente', cook: 'Cocinar conmigo', favorite: 'Favorito', noFav: 'Todavía no hay favoritos. Toca el corazón de una comida que quieras repetir.', noHistory: 'Todavía no hay historial.',
    ...
    scanError: 'No pude leer esa foto. Intenta otra o escribe los ingredientes.', error: 'Algo salió mal. Intenta de nuevo.', saved: 'Guardado', profileSaved: 'Perfil familiar guardado.', noPhoto: 'Las fotos están por agregarse. La receta ya está lista para cocinar.', newRecipe: 'Comenzar receta nueva',
...
function App() {
  const [lang, setLang] = useState<Lang>(() => readLocal('listomeal-lang', 'en' as Lang));
      ...
      <header className="sticky top-0 z-20 border-b border-orange-100 bg-[#fffaf2]/95 backdrop-blur">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="flex items-center gap-2 rounded-full border border-orange-200 bg-white px-3 py-2 text-sm font-bold shadow-sm" onClick={() => setLang(lang === 'en' ? 'es' : 'en')}><Languages size={16} /> {lang === 'en' ? 'ES' : 'EN'}</button>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-5xl px-4 py-5">
        {tab === 'home' && <>{recipes.length === 0 ? (
          <section className="mx-auto max-w-5xl">
            <div className="mb-5 overflow-hidden rounded-[2rem] shadow-xl shadow-orange-100"><img src="/resources/hero-banner.jpg" alt="ListoMeal — Turn what you have into something amazing" className="block h-auto w-full" loading="eager" /></div>
...
function RecipePhoto({ title, fallback }: { title:string; fallback:string }) {
  const [photo,setPhoto]=useState<{imageUrl:string;photographer:string;photographerUrl:string;pexelsUrl:string}|null>(null);const [loaded,setLoaded]=useState(false);
  useEffect(()=>{let active=true;setPhoto(null);setLoaded(false);void api.post('/api/photo',{title}).then(response=>{if(!active)return;const data=response.data as {imageUrl?:string;photographer?:string;photographerUrl?:string;pexelsUrl?:string};if(data.imageUrl)setPhoto({imageUrl:data.imageUrl,photographer:data.photographer||'Pexels',photographerUrl:data.photographerUrl||'https://www.pexels.com',pexelsUrl:data.pexelsUrl||'https://www.pexels.com'});}).catch(()=>undefined);return()=>{active=false;};},[title]);
  if(!photo)return <div className="grid h-full place-items-center text-center"><div><ChefHat className="mx-auto mb-2 text-orange-400" size={42}/><div className="text-xs font-bold uppercase tracking-widest text-orange-500">{fallback}</div></div></div>;
  return <div className="relative h-full w-full">{!loaded&&<div className="absolute inset-0 animate-pulse bg-orange-50"/>}<img src={photo.imageUrl} alt={title} className="h-full w-full object-cover" onLoad={()=>setLoaded(true)}/><div className="absolute bottom-0 left-0 right-0 bg-black/55 px-2 py-1 text-[10px] text-white">Photo by <a className="underline" href={photo.photographerUrl} target="_blank" rel="noreferrer">{photo.photographer}</a> on <a className="underline" href={photo.pexelsUrl} target="_blank" rel="noreferrer">Pexels</a></div></div>;
}
